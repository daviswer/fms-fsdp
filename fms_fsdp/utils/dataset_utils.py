...
class LoaderMonitor():
    ...
    def apply_delta(self, delta, state):
        for k in delta.keys():
            ...
            # Apply deltas
            assert delta[k][2].dtype==state[k].dtype, (k, delta[k][2][0], state[k][0])
...
def save_distributed_state_dict(loader: StatefulDataLoader, path: str, device_mesh=None, placements=None):
    ...
    print(rank, dstate['SamplingDataset.states.StreamingDocDataset.lcg_state'].to_local())
...
def load_distributed_state_dict(loader: StatefulDataLoader, path: str, device_mesh=None, placements=None):
    ...
    print(rank, dstate['SamplingDataset.states.StreamingDocDataset.lcg_state'])
    dstate = {k:v.to_local().chunk(nworkers) for k,v in dstate.items()}
