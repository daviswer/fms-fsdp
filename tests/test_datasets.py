...
def test_two_loader():
    ...
    two_loader_check(functools.partial(basic_scalable, n_logical_shards=10))
    two_loader_check(basic_sampler)
    two_loader_check(functools.partial(basic_sampler_scalable, n_logical_shards=10))
...
def test_reload_epoch():
    ...
    reload_epoch_check(functools.partial(basic_scalable, n_logical_shards=10))
    reload_epoch_check(basic_sampler)
    reload_epoch_check(functools.partial(basic_sampler_scalable, n_logical_shards=10))
...
def test_reload_complete_epoch():
    ...
    reload_single_epoch_check(functools.partial(basic_scalable, n_logical_shards=10))
    reload_single_epoch_check(basic_sampler)
    reload_single_epoch_check(
        functools.partial(basic_sampler_scalable, n_logical_shards=10)
    )
...
def test_checkpoint_reload_match():
    ...
    datasets = [
        CheckpointDataset(x, os.path.join(tmpdir.name, "ckp_test"), 90, 0.5)
        for x in datasets
    ]
    loaders = [
    ...
    for d in datasets2:
        assert (
            d.step * d.spc == 90
        ), f"Expected to load back to step 90, got {d.step*d.spc}"
